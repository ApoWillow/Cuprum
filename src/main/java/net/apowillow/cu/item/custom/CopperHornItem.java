...
public class CopperHornItem extends Item {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        ...
        if (optional.isPresent()) {
            MutableText mutableText = Text.translatable(Util.createTranslationKey(COPPER_HORN_INSTRUMENT, optional.get().getValue()));
    ...
    private Optional<? extends RegistryEntry<CopperHornInstrument>> getInstrument(ItemStack stack) {
        ...
        if (nbtCompound != null && (identifier = Identifier.tryParse(nbtCompound.getString(COPPER_HORN_INSTRUMENT))) != null) {
            return CopperHornRegistries.COPPER_HORN_INSTRUMENT.getEntry(RegistryKey.of(CopperHornRegistries.COPPER_HORN_INSTRUMENT_KEY, identifier));
        }
        Iterator<RegistryEntry<CopperHornInstrument>> iterator = CopperHornRegistries.COPPER_HORN_INSTRUMENT.iterateEntries(this.instrumentTag).iterator();
        if (iterator.hasNext()) {
